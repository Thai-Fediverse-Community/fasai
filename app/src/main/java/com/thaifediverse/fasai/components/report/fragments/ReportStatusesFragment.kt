...
package com.thaifediverse.fasai.components.report.fragments
...
import com.thaifediverse.fasai.components.report.ReportViewModel
import com.thaifediverse.fasai.components.report.Screen
import com.thaifediverse.fasai.components.report.adapter.AdapterHandler
import com.thaifediverse.fasai.components.report.adapter.StatusesAdapter
...
class ReportStatusesFragment : Fragment(), Injectable, AdapterHandler {
    ...
    private fun initStatusesView() {
        ...
        viewModel.networkStateAfter.observe(viewLifecycleOwner, Observer {
            if (it?.status == com.thaifediverse.fasai.util.Status.RUNNING)
                progressBarBottom.show()
            else
                progressBarBottom.hide()

            if (it?.status == com.thaifediverse.fasai.util.Status.FAILED)
                showError(it.msg)
        })
        ...
        viewModel.networkStateRefresh.observe(viewLifecycleOwner, Observer {
            if (it?.status == com.thaifediverse.fasai.util.Status.RUNNING && !swipeRefreshLayout.isRefreshing)
